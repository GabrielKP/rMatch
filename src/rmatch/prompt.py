...
def prompt_default_with_story(
    ...
    # format story
    story_str = format_story_segments(story_segments)
...
def prompt_default_with_story_cot(
    story_segments: list[str],
    recall_segments: list[str],
    target_idx: int,
    window_size: int,
...
<reasoning>
The target segment contains three claims:
1. "She laughed" — matches element 4 (Sarah laughed and said it was fine).
2. "the barista spilled the drink" — matches element 3 (The barista spilled the drink).
3. "because she is a kind person" — not stated in any story element.
</reasoning>
<3,4>
</EXAMPLE>
...
def prompt_default_cot_no_example(
    story_segments: list[str],
    recall_segments: list[str],
    target_idx: int,
    window_size: int,
...
First, briefly state what the target segment conveys and which story elements it maps to. Then list the matched element numbers.
...
def prompt_alternative_cot(
    ...
    Tehn return a set of numbers in angle brackets, for example:
    <3, 7, 12>""",
        _parse_output,
    )


def prompt_alternative_no_story(
    story_segments: list[str],
    recall_segments: list[str],
    target_idx: int,
    window_size: int,
...
def get_prompt_and_parser(
    ...
    elif prompt_type == "default_with_story":
        return prompt_default_with_story(
            story_segments, recall_segments, target_idx, window_size, verbose_errors
        )
    elif prompt_type == "default_cot":
        return prompt_default_cot(
            story_segments, recall_segments, target_idx, window_size, verbose_errors
        )
    elif prompt_type == "default_cot_no_example":
        return prompt_default_cot_no_example(
            story_segments, recall_segments, target_idx, window_size, verbose_errors
        )
    elif prompt_type == "default_with_story_cot":
        return prompt_default_with_story_cot(
            story_segments, recall_segments, target_idx, window_size, verbose_errors
        )
    elif prompt_type == "alternative":
        return prompt_alternative(
            story_segments, recall_segments, target_idx, window_size, verbose_errors
        )
    elif prompt_type == "alternative_cot":
        return prompt_alternative_cot(
            story_segments, recall_segments, target_idx, window_size, verbose_errors
        )
